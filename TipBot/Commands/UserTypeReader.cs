...
using Discord.Rest;
namespace TipBot.Commands {
    public class UserTypeReaderExtended<T> : TypeReader
            where T : class, IUser {
        /// <inheritdoc />
        public override async Task<TypeReaderResult> ReadAsync(ICommandContext context, string input, IServiceProvider services) {
            ...
            return await ReadAsyncDownload(context, input, services);
            //return TypeReaderResult.FromError(CommandError.ObjectNotFound, "User not found.");
        }

        public async Task<TypeReaderResult> ReadAsyncDownload(ICommandContext context, string input, IServiceProvider services) {
            var results = new Dictionary<ulong, TypeReaderValue>();
            IAsyncEnumerable<IUser> channelUsers = context.Channel.GetUsersAsync(CacheMode.AllowDownload).Flatten(); // it's better
            IReadOnlyCollection<IGuildUser> guildUsers = ImmutableArray.Create<IGuildUser>();
            ...
            if (MentionUtils.TryParseUser(input, out var id)) {
                if (context.Guild != null)
                    ...
                    AddResult(results, channel.GetUser(id) as T, 1.00f);
            }
            ...
            if (ulong.TryParse(input, NumberStyles.None, CultureInfo.InvariantCulture, out id)) {
                if (context.Guild != null)
                    ...
                    AddResult(results, channel.GetUser(id) as T, 0.90f);
            }
